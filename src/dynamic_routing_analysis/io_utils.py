...
def pupil(kernel_name, session, fit, behavior_info):
    ...
    return bin_timeseries(df.pupil_area.values, df.timestamps.values, fit['timebins_all'])


def running(kernel_name, session, fit, behavior_info):
    return bin_timeseries(session._running_speed.data, session._running_speed.timestamps, fit['timebins_all'])
...
def LP_features(kernel_name, session, fit, behavior_info):
    ...
    return bin_timeseries(part_xy, timestamps, fit['timebins_all'])
...
# def toeplitz_this_kernel(this_kernel, kernel_length_samples, offset_samples, epoch_trace):
#     top_kernel = np.zeros((len(epoch_trace), kernel_length_samples)) + np.nan
#     unique_epochs = np.array([epoch_trace[i] for i in sorted(np.unique(epoch_trace, return_index=True)[1])])
#     for epoch in unique_epochs:
#         ind = np.where(epoch_trace == epoch)[0]
#         top_kernel[ind] = toeplitz(this_kernel[ind], kernel_length_samples, offset_samples).T
#     return top_kernel.T
...
# def bin_timeseries2(x, x_timestamps, timebins):
#     Tm = timebins.shape[0]
#     start_indices = np.searchsorted(x_timestamps, timebins[:, 0], side='left')
#     end_indices = np.searchsorted(x_timestamps, timebins[:, 1], side='right')
#
