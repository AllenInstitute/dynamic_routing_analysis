...
def get_spike_counts(spike_times, timebins):
    ...
    '''
    # - dtype is float for consistency with previous version
    # - operations with other float arrays also seems to be faster than converting from int to float
    return np.array(
        [b - a for a, b in np.searchsorted(spike_times, timebins)], dtype=np.float64
    )
...
def pupil(kernel_name, session, fit, behavior_info):
    ...
    df = process_pupil_data(
        dr_datacube.get_lf(
            '/processing/behavior/eye_tracking', session_id=session, nwb=True
        ).collect().to_pandas(),
        behavior_info,
    )
...
def running(kernel_name, session, fit, behavior_info):
    timeseries = dr_datacube.get_lf(
        '/processing/behavior/running_speed', session_id=session, nwb=True
    ).collect().to_pandas()
